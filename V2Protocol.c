...
const struct {
	uint8_t Header[3];
	uint8_t Desc[];
} SignonResponse PROGMEM = {Header: {AICB_CMD_SIGN_ON, AICB_STATUS_CMD_OK, (sizeof("AVRISP_2") - 1)}, Desc: "AVRISP_2"};
...
static void V2P_ProcessPacketData(FuncPtr PacketDecodeFunction)
{
	...
	{ 
		...
		case AICB_CMD_SET_PARAMETER:						
			V2P_GetSetParameter();
			break;
		case V2P_CMD_DUMP_DATAFLASH:           // Buttload-only command - dump dataflash contents
			...
			break;
		case V2P_CMD_DUMP_DATAFLASH_RAW:
			DF_ENABLEDATAFLASH(TRUE);
			SPI_SPIInit();
